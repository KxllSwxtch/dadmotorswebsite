...
const CatalogClient = () => {
  ...
  return (
    <div className="md:mt-40 mb-30 mt-35 px-6">
      ...
      <div className="container m-auto grid grid-cols-1 md:grid-cols-5 md:gap-15">
        ...
        {loading ? (
          ...
          <div className="md:col-span-4 flex flex-col items-center justify-center w-full mt-10 py-12 px-6 bg-gray-100 rounded-lg border border-gray-300">
            <p className="text-xl font-semibold text-gray-700 text-center mb-6">
              {error ||
                "Приносим свои извинения но на данный платформе нет подходящих вариантов\nНе спешите растраиваться свяжитесь с нашим менеджером он обязательно подберет автомобиль вашей мечты"}
            </p>
            <button
              className="bg-red-500 text-white py-3 px-6 rounded-md hover:bg-red-600 transition cursor-pointer font-medium"
              onClick={() => {
                setSelectedManufacturer("")
                setSelectedModelGroup("")
                setSelectedModel("")
            ...
            >
              Сбросить фильтры и начать поиск заново
            </button>
          </div>
        )}
      </div>
